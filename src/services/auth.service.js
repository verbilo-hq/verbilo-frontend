...
import { isDemoMode, hasLocalMockCognito } from "../lib/demoMode.js";
...
// Demo surfaces (local dev, and the public demo.* host) authenticate against
// the fixture roster instead of Cognito - see lib/demoMode.js.
const USE_MOCK_COGNITO = isDemoMode();
...
/** Mint an unsigned, JWT-shaped token locally. Used on deployed demo hosts,
 *  where no mock-Cognito server exists. It is never verified client-side;
 *  tokenIsExpired() only reads its `exp`. Backend calls made with it will
 *  401, which is expected - the demo runs on the localStorage engine. */
function mintLocalDemoToken(account) {
  const b64url = (obj) =>
...
async function mintMockToken(account) {
  // Deployed demo hosts have no mock-Cognito server - mint locally instead.
  if (!hasLocalMockCognito()) return mintLocalDemoToken(account);
  try {
    ...
    if (!res.ok) throw new Error(`mock-cognito returned ${res.status}`);
    const { access_token } = await res.json();
    return access_token;
  } catch {
    // Mock server down: fall back so the demo still signs in.
    return mintLocalDemoToken(account);
  }
}
